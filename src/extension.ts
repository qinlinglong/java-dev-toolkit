...
function parseRestPath(path: string): string[] {
    // 移除开头的斜杠并分割路径段
...
// 查找匹配的端点
async function findMatchingEndpoints(javaFiles: vscode.Uri[], targetPathSegments: string[]): Promise<EndpointMatch[]> {
...
// 计算模糊匹配得分（支持全模糊匹配）
function calculateFuzzyMatchScore(endpointPath: string, targetPathSegments: string[]): number {
...
function fuzzyStringMatch(target: string, source: string): number {
    if (!target || !source) {
        return 0;
    }
...
// 提取方法名
function extractMethodName(lines: string[], position: number): string | null {
    // 向前搜索找到方法声明
    for (let i = position; i >= 0; i--) {
...
// 导航到端点
async function navigateToEndpoint(match: EndpointMatch) {
    try {
